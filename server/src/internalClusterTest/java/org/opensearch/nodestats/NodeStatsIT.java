...
public class NodeStatsIT extends OpenSearchIntegTestCase {
    ...
    private static final Map<String, AtomicLong> expectedDocStatusCounter;

    static {
        expectedDocStatusCounter = new HashMap<>();
        expectedDocStatusCounter.put("2xx", new AtomicLong(0));
        expectedDocStatusCounter.put("4xx", new AtomicLong(0));
    }
    ...
    public void testNodeIndicesStatsBulk() {
        ...
        for (BulkItemResponse itemResponse : response.getItems()) {
            expectedDocStatusCounter.get(itemResponse.getResponse().status().getClassName()).incrementAndGet();
        }
        ...
        Map<String, AtomicLong> docStatusCounter = stats.getDocStatusStats().getDocStatusCounter();

        for (String key : docStatusCounter.keySet()) {
