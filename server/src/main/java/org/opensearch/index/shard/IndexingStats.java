...
public class IndexingStats implements Writeable, ToXContentFragment {
    ...
    public static class Stats implements Writeable, ToXContentFragment {
        ...
        public static class DocStatusStats implements Writeable, ToXContentFragment {
            ...
            // Counter map for rest category class place value number
            private final Map<Short, AtomicLong> docStatusCounter;
            ...
            public DocStatusStats(Map<Short, AtomicLong> docStatusCounter) {
            ...
            public DocStatusStats(StreamInput in) throws IOException {
                ...
                for (int i = 0; i < size; ++i) {
                    docStatusCounter.put(in.readShort(), new AtomicLong(in.readLong()));
                }
            }
            ...
            public void add(final DocStatusStats stats) {
                if (null == stats || null == stats.docStatusCounter) {
                    return;
                }

                for (Map.Entry<Short, AtomicLong> entry : stats.docStatusCounter.entrySet()) {
                    short k = entry.getKey();
             ...
             */
            public void inc(final int status) {
                short category = (short)(status / 100);

                docStatusCounter.computeIfAbsent(category, x -> new AtomicLong(0));
                docStatusCounter.get(category).incrementAndGet();
            }

            public Map<Short, AtomicLong> getDocStatusCounter() {
            ...
            public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
                ...
                for (Map.Entry<Short, AtomicLong> entry : docStatusCounter.entrySet()) {
                    builder.field(String.format(Locale.ROOT, "%dxx", entry.getKey()), entry.getValue().longValue());
                }
